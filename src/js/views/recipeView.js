// A package to express decimals as more readable fractions
...
class RecipeView extends View {
        ...
        <ul class="recipe__ingredient-list">
          ${
            ...
            this._data.ingredients
              .map(ing => this._generateIngHTML(ing))
              .join('\n')
          }
        </ul>
      </div>
  ...
  // Adding subscribers to the events that this view emits
  // Essentially, adding handler to render the view when certain events happen
  ...
  addHandlerBookmark(handler) {
    // Must use event delegation here. Bookmark button may not be present yet upon opening the
    // page, so its impossible to attach an event listener to it right
    this._parentElement.addEventListener('click', function (e) {
